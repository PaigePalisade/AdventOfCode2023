...
int main() {
    ...
    for (int i = 0; i < NUM_HANDS; i++) {
        ...
        for (int j = 0; j < 256; j++) {
            if (count[j] > maxCount && j != 'J') {
                maxCount = count[j];
                maxCountInd = j;
            }
        }
        for (int j = 0; j < 256; j++) {
            if (count[j] > secondMaxCount && j != maxCountInd && j != 'J') {
        ...
        else if (maxCount + numJs == 4) {
            hands[i].type = 6;
            printf("Four of a kind\n");
        }
        else if ((maxCount + numJs == 3 && secondMaxCount == 2) || (maxCount == 3 && secondMaxCount + numJs == 2)) {
            hands[i].type = 5;
            printf("Full house\n");
        }
        else if (maxCount + numJs == 3) {
            hands[i].type = 4;
            printf("Three of a kind\n");
        }
        else if ((maxCount + numJs == 2 && secondMaxCount == 2) || (maxCount == 2 && secondMaxCount + numJs == 2)) {
            hands[i].type = 3;
            printf("Two Pair\n");
        }
        else if (maxCount + numJs == 2) {
            hands[i].type = 2;
            printf("One Pair\n");
        }
        else {
